...
print(head(yearly.reports))

models <- list(
    p = glm(reports ~ social_accounts, data = yearly.reports, family = "poisson"),
    nb = glm.nb(reports ~ social_accounts, data = yearly.reports),
    ts.lin.p = tsglm(
        yearly.reports$reports,
        model = list(past_obs = c(1), past_mean = NULL, external = NULL),
        xreg = NULL,
        link = "identity",
        distr = "poisson"
    ),
...
plot(yearly.reports$reports, type="b", ylim=c(0, 50000000))
points(fitted(models$p), pch=16, col="#ff4020")
points(fitted(models$nb), pch=16, col="#40a020")
...
# 
#     model <- models[[name]]
#     cat(paste0("Processing model ", name, "\n"))
#     
