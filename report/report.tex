...
since the author worked as a software engineer for Facebook from 2018 to 2019.
However, the author did \emph{not} work on content moderation and does
\emph{not} retain any financial interest in the firm.

In looking for such \emph{scrappy audits}, we soon realized that redundant and
...
didn't look it at first. After refactoring the code for analyzing Meta's \V{CSV}
files, we carefully compared outputs to ensure that the new version still worked
as expected. When numbers didn't match, we first reviewed the code and, finding
nothing wrong, next checked the programs' input. When that turned out to be the
\V{CSV} file for \V{Q3}~2022 instead of \V{Q2}~2022, we compared all quantities
between the two files besides values newly added in \V{Q3} and discovered that
113 out of 2,633 or 4.3\% of quantities differed, 77 of them dating back to
\V{Q4}~2020. We had found the cause for the divergent outputs---and the second
audit for this paper!
...
sexual exploitation of minors, you wouldn't even need the latter, since we
...
statistics~\cite{TrujilloFagniea2024}, we am not aware of similar studies.
...
In selecting platforms for my scrappy audits, we started with Buffer's list of
...
contextualize my overview, we also draw on the computer-science literature about
